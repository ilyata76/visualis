...
std::string vampire5::parser::get_out(const int& number) {
	...
	Assert(atoms_file.is_open(), 101, "atoms_file не открылся", "Assert. ФУНКЦИЯ: vampire5::parser::get_out");
	Assert(spins_file.is_open(), 101, "spins_file не открылся", "Assert. ФУНКЦИЯ: vampire5::parser::get_out");
	...
	Assert( std::stoi(str1) == std::stoi(str2), 102, "str1 != str2, разное количество строк в файлах", 
											   "Assert. Проверьте первую строку обоих файлов. ФУНКЦИЯ: vampire5::parser::get_out" );
